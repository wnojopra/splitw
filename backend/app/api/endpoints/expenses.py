...
def get_group_balances_and_debts(
    ...
    # 1. Loop through non-deleted expenses and update balances grouped by currency
    expenses = crud.get_group_expenses(db, group_id=group_id)
    
    balances_by_currency = {}
    for expense in expenses:
        curr = expense.currency or "USD"
        if curr not in balances_by_currency:
            balances_by_currency[curr] = {member.id: Decimal("0.00") for member in db_group.members}
            
        payer_id = expense.paid_by_id
        amount = Decimal(str(expense.amount))
        
        # Payer's balance increases by the paid amount
        if payer_id in balances_by_currency[curr]:
            balances_by_currency[curr][payer_id] += amount
            
        # Subtract owed amounts for each participant in splits
        for split in expense.splits:
            debtor_id = split.user_id
            owed = Decimal(str(split.owed_amount))
            if debtor_id in balances_by_currency[curr]:
                balances_by_currency[curr][debtor_id] -= owed
                
    # Convert balances dict to JSON-friendly string format for transmission
    json_balances = {
        curr: {uid: float(bal) for uid, bal in b_dict.items()}
        for curr, b_dict in balances_by_currency.items()
    }
    
    # 2. Debt Simplification Algorithm per currency
    simplified_debts = []
    
    for curr, balances in balances_by_currency.items():
